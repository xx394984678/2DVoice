...
//            }
        ...
        if (!isMotionUpdated) {
            if (!isBlinking) {
                ...
                blinking();
                if (isBlinkLeft) {
                    CubismId eyeBlinkId = eyeBlinkIds.get(0);
                    model.setParameterValue(eyeBlinkId, blinkValue);
                } else {
                    CubismId eyeBlinkId = eyeBlinkIds.get(1);
                    model.setParameterValue(eyeBlinkId, blinkValue);
                }
            }
        }
        ...
        if (lipSync) {
            ...
            for (int i = 0; i < lipSyncIds.size(); i++) {
                ...
                model.setParameterValue(lipSyncId, lipValue);
            }
        }
    ...
    //嘴巴相关
    private float lipValue = 1.0f;
    private final float lipOnceValue = 0.07f;
    private boolean isClosingLip = true;

    private void lipSync(){
        if (isClosingLip) {
            lipValue -= lipOnceValue;
            if (lipValue <= 0.0f) {
                isClosingLip = false;
                lipValue = 0.0f;
            }
    ...
    private boolean isBlinkLeft = false;
    private float blinkValue = 1.0f;
    private boolean isBlinkClosing = true;
    ...
    public void blinkOnce(){
        ...
        isBlinking = true;
        isBlinkLeft = kotlin.random.Random.Default.nextBoolean();
    }
    ...
    public int startMotion(final String group,
        ...
        Log.d("lr_test", "startMotion: " + name);//3 笑一下  2开心说话， 4抬手刷花   3闭眼一下说话   1 好奇但是不说话 5震惊  6抬手震惊不说话
        if (motion == null) {
    ...
    public int startRandomMotion(final String group, int priority, IFinishedMotionCallback onFinishedMotionHandler, IBeganMotionCallback onBeganMotionHandler) {
        ...
        if (group.equals("TapBody")) {
            return startMotion(group, 0, priority, onFinishedMotionHandler, onBeganMotionHandler);
        }
        return startMotion(group, testNumber, priority, onFinishedMotionHandler, onBeganMotionHandler);
    }
